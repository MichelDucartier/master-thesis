...
\begin{table}
    ...
    \begin{tabular}{ccc}
        ...
        \midrule
        Llama CLIP & LLaMa 3.1 Instruct 8B & clip-vit-base-patch32
 \\
        Llama BiomedCLIP & LLaMa 3.1 Instruct 8B & BiomedCLIP \\
        Llama ATTN-PEP & LLaMa 3.1 Instruct 8B & MultiMeditron ATTN-PEP \\
        Qwen3 BiomedCLIP & Qwen3-4B & BiomedCLIP \\
        Apertus BiomedCLIP & Apertus-8B & BiomedCLIP \\
        \bottomrule
    \end{tabular}
...
We conducted 6 experiments to test the performance of MultiMeditron. The details of the 6 experiments are listed in Table \ref{tab:experiments}. Llama CLIP acts as our baseline. We attempt to increase the performance of MultiMeditron by:
\begin{enumerate}
    \item Changing the vision encoder to a medically specialized one with either BiomedCLIP or our modality-driven experts
    \item Changing the base LLM architecture, from Llama 3.1 Instruct 8B to Qwen3-4B or Apertus-8B
\end{enumerate}

Multimodal models trained with medically specialized vision embedders outperform those using general-purpose vision embedders \cite{sellergren2025medgemmatechnicalreport}. Accordingly, we adopt BiomedCLIP in our architectures when replacing the base LLM with Qwen3-4B or Apertus Instruct 8B.

BiomedCLIP is trained exclusively on open data from PubMed Central and is built upon fully open models, including PubMedBERT a Vision Transformer pretrained on general-domain public data \cite{zhang2025biomedclipmultimodalbiomedicalfoundation,dosovitskiy2020vit}.
...
We evaluate a MultiMeditron by computing the conditional likelihood of a predefined answer (e.g., a multiple-choice option) given a multimodal prompt consisting of text and images.
...
\begin{itemize}
    ...
    \item Out of a set of options, we consider that the model answer option $y$ if the negative log likelihood
\end{itemize}
...
\begin{itemize}
    ...
    \item $z_t = (z_{t,1}, \dots, z_{t,V})$ be the logits where $z_t$ are the logits for output token $t \in [1, T]$ and $V$ is the size of the vocabulary
\end{itemize}
...
For example, we extract the LLM answer using a regular expression that captures the text following the \texttt{"Answer:"} or \texttt{"The answer is"} pattern in the model's generated response. We then compute the accuracy by comparing the extracted answers to the ground truth answers provided in the benchmark datasets. The accuracy is then reported for each model and each benchmark.
...
To address this issue, we remove the CoT prompting and instruct the model to produce answers that are as concise as possible. The revised prompt templates for SLAKE and PathVQA adopt a question-answering format, requiring the model to output a single word or short phrase corresponding to the correct answer.
