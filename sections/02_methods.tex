...
\begin{itemize}
    \item \textbf{AVG-PEP} (Average and Per-Expert-Projection): Each expert has its own projection layer to project its embedding into the LLM token space. We then perform a weighted average on the projected embeddings using the weights produced by the gating network to obtain the final embedding.
...
We conducted 8 full training to test the performance of MultiMeditron on different configuration. The details of the experiments are listed in Table \ref{tab:experiments}.
