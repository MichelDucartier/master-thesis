...
\begin{itemize}
    ...
    \item \textbf{LLM}: This module integrates the modality embeddings with text data to perform downstream tasks such as classification or generation.
\end{itemize}
...
In Figure \ref{fig:modality_data_flow}, we illustrate the data flow for a generic modality as well as a specific example for image data. The data flow consists of four main components:
\begin{itemize}
    ...
    \item \textbf{LLM}: This component integrates the modality embeddings with text data to perform downstream tasks such as classification or generation.
\end{itemize}


\section{Additional results}

\subsection{Training with a Reasoning Language Model}

In order to integrate the latest language models in MultiMeditron, we tried to use a reasoning model as our language model. Specifically, we trained with Qwen3-8B as our base LLM with the same data and hyperparameters as presented in \ref{sec:training}.

We compare inference results between Qwen-4B-Instruct, Llama3.1-8B and Qwen3-8B
