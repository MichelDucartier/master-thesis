...
Current open-source frameworks for multimodal AI research often lack flexibility and extensibility, making it difficult to customize and scale solutions for the requirements of clinical applications.
...
Taking inspiration from these advancements, we propose MultiMeditron, a modular multimodal pipeline specifically tailored for medical applications. A key innovation of MultiMeditron is its modality-driven image embedder, which customizes embeddings based on the type of image received in input.

We demonstrate that, despite being trained on only open-licensed medical data, the MultiMeditron suite of models achieves competitive results in multimodal medical benchmarks while also providing a modular research framework for multimodal training and evaluation. Although our modality-driven approach performs slightly worse than the standard approach, it still achieves reasonable results on those same benchmarks.
...
\begin{enumerate}
    ...
    \item To evaluate the performance of MultiMeditron on established medical multimodal benchmarks (addressed in Section \ref{sec:results}).
    \item To ensure reproducibility and facilitate collaborative research by providing comprehensive documentation and Docker images (addressed in Section \ref{sec:architecture_philosophy} and Section \ref{sec:add_modality}).
\end{enumerate}
...
With MultiMeditron, we do not only introduce a new multimodal pipeline but also set a new standard for modularity, extensibility, and reproducibility in medical AI research. We hope that MultiMeditron will serve as a foundational tool for researchers and practitioners, accelerating the development of robust multimodal AI solutions that can effectively address complex clinical challenges and ultimately improve patient care.
