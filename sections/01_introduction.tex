...
% The roots of multimodal models can be traced back to early attempts to combine textual and visual data for various tasks such as classification and retrieval. Over time, these models evolved to integrate more diverse modalities like audio, video, and sensor data, driven by the realization that real-world understanding often requires synthesizing information from multiple sources.
...
\begin{enumerate}
    ...
    \item To evaluate the performance of MultiMeditron on established medical multimodal benchmarks (addressed in Section \ref{sec:results1}).
    \item To ensure reproducibility and facilitate collaborative research by providing comprehensive documentation and Docker images (addressed in Section \ref{sec:results2}).
\end{enumerate}
